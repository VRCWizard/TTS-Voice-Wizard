...
namespace TTS
{
    public class MikuTTS
    {
        public static async Task MikuTextAsSpeech(string text, CancellationToken ct = default)
        {
            ...
            System.Diagnostics.Debug.WriteLine("miku speech ran " + voice);
            try
           {
                byte[] result = await CallMikuAPIAsync(text);     
           ...
           {
                OutputText.outputLog("[Miku TTS Error: " + ex.Message + "]", Color.Red);
        ...
        public static async Task<byte[]> CallMikuAPIAsync(string text)
        {
            ...
            WebRequest request = WebRequest.Create("http://aikelab.net/mikugo/api.cgi"+"?chouon="+chouon+"&bunsetsu="+ bunsetsu+ "&romaji="+ romaji+ "&encoding="+ encoding + "&type=" + type + "&sentence=" + text);
            request.Method = "GET";
